...
Skill 默认**自动路由**：每个请求会与已加载 skill 的 `whenToUse` 描述匹配（输入中显式提到 skill 名可立即命中；否则用一次小规模确定性 LLM 调用选出最匹配者）。命中则应用该 skill 的 persona、工具与执行模式；不命中则回退为直接对话——skill 绝不会被强加到无关输入上。锁定某个 skill（设置菜单 → "Skill 路由"，或 `--skill <名称>` 启动）只是把候选集收窄到这一个，命中更快——**并不强制每个请求都走它**：无关输入照样直接回答。
...
模型端点为 OpenAI 兼容（`/v1/chat/completions`，支持流式）。除监听地址外，所有配置修改在下一个 chat 请求时热重载——无需重启。可通过设置面板「打开配置文件」按钮用系统编辑器编辑。

### 修改监听地址（绑定 IP / 端口）

默认只监听本机回环地址 `127.0.0.1:3081`。如需局域网或其他设备访问，编辑配置文件中的 `server.listen`：

```yaml
server:
  listen: "0.0.0.0:3081"   # 监听所有网卡（局域网可访问）
  # listen: "192.168.1.100:3081"   # 或绑定指定网卡 IP
  # listen: "127.0.0.1:3081"       # 默认：仅本机
```

> ⚠️ **监听地址修改后必须重启进程才生效**（服务器在启动时绑定端口，不支持热重载）。改完保存后按 `Ctrl+C` 停止并重新运行 `dsh-lite`。
>
> **注意**：`server.listen` 与运行中的 dsh-lite 可执行文件**同目录**的 `config.yaml` 生效（设置面板显示的路径即实际文件）。若存在多个 config.yaml（如 `release/` 与 `target/release/`），请以设置面板「设置 → 配置文件路径」显示的为准。
