...
Skill 默认**自动路由**：每个请求会与已加载 skill 的 `whenToUse` 描述匹配（输入中显式提到 skill 名可立即命中；否则用一次小规模确定性 LLM 调用选出最匹配者）。命中则应用该 skill 的 persona、工具与执行模式；不命中则回退为直接对话——skill 绝不会被强加到无关输入上。如需永久锁定某个 skill（关闭路由，适合固定流程提速），在设置菜单 → "Skill 路由"中选择，或用 `--skill <名称>` 启动。
