...
Skills are auto-routed by default: each request is matched against the loaded skills' `whenToUse` descriptions (an explicit skill-name mention wins instantly; otherwise a tiny deterministic LLM call picks the best fit). Matches apply the skill's persona, tools, and execution mode; non-matches fall back to plain conversation — the skill is never forced onto unrelated input. Locking a skill (Settings → Skill routing, or `--skill <name>`) narrows the candidate set to that one skill for faster matching — it does **not** force every request through it: unrelated input still gets a direct answer.
