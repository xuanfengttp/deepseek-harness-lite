...
impl AgentLoop {
    ...
    pub async fn run_turn(
        ...
        // Model-switch notice (upstream `installModelSelection` announce):
        // If this session was previously driven by a different model, insert a
        // model-visible user message so the newly selected model can interpret
        // earlier assistant turns (e.g. image placeholders left by a text-only
        // model). Initial selection and unchanged routes add no notice.
        let previous_model = self.session.last_model().map(String::from);
        if let Some(prev) = previous_model.as_deref() {
            if !prev.is_empty() && prev != self.model {
                let notice = format!(
                    "[model changed: assistant turns above this point were generated by {prev}; the session continues with {}]",
                    self.model
                );
                log::info!("Model switch {prev} -> {} — emitting notice", self.model);
        ...
        // Record the durable request route (which model drove this turn).
        self.session
            .append(SessionEvent::RequestHeader { model: self.model.clone() });
        self.session.set_last_model(&self.model);
